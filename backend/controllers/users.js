  ...
  try {
    ...
    if (!isMatched) {
      ...
      res.status(200).cookie('jwt', token, {
        maxAge: 3600000 * 24 * 7, httpOnly: true, domain: '.nomoredomains.club', secure: true, sameSite: 'none',
      })
...
const logout = async (req, res, next) => {
  try {
    ...
    res.status(200).clearCookie('jwt', {
      httpOnly: true, domain: '.nomoredomains.club', secure: true, sameSite: 'none',
    })
